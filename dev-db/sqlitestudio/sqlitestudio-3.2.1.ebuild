...
src_configure() {
	...
	## Core
	rmkdir "${MY_CORE_BUILD_DIR}"
	cd "${MY_CORE_BUILD_DIR}" || die
	eqmake5 "${qmake_args[@]}" "${MY_CORE_SRC_DIR}"

	## Plugins
	rmkdir "${MY_PLUGINS_BUILD_DIR}"
	cd "${MY_PLUGINS_BUILD_DIR}" || die
...
src_install() {
	...
	local -a make_desktop_entry_args=(
		"${EPREFIX}/usr/bin/${PN} -- %F"  # exec
		'SQLiteStudio3'  # name
		"${PN}"  # icon
		'Development;Database;Utility'  # categories
	)
	local -a make_desktop_entry_extras=( 'MimeType=application/x-sqlite3;' )
